...
public class Handler_Items {
    ...
    private static void registerModel(String path, String parent) {
        ...
        RESOURCE_PACK.addModel(JModel.model(parent).textures(JModel.textures().layer0("eltcore:item/" + path)), resourceId);
    }

    private static void registerModel(String path) {
        Identifier resourceId = new Identifier("eltcore", "item/" + path);
        RESOURCE_PACK.addModel(JModel.model("item/generated").textures(JModel.textures().layer0("eltcore:item/" + path)), resourceId);
    }
    ...
    private static void registerLang(String path, String language, String name, String tooltip) {
        ...
        if (path.contains("/")) {
            ...
            if (language.equalsIgnoreCase("en_us"))
                RESOURCE_PACK.addLang(new Identifier("eltcore", language), EN_US.translate("item.eltcore." + unlocalizedPath, name).translate("item.eltcore." + unlocalizedPath + ".tooltip", tooltip));
            if (language.equalsIgnoreCase("zh_cn"))
                RESOURCE_PACK.addLang(new Identifier("eltcore", language), ZH_CN.translate("item.eltcore." + unlocalizedPath, name).translate("item.eltcore." + unlocalizedPath + ".tooltip", tooltip));
        } else {
            if (language.equalsIgnoreCase("en_us"))
                RESOURCE_PACK.addLang(new Identifier("eltcore", language), EN_US.translate("item.eltcore." + path, name).translate("item.eltcore." + path + ".tooltip", tooltip));
            if (language.equalsIgnoreCase("zh_cn"))
                RESOURCE_PACK.addLang(new Identifier("eltcore", language), ZH_CN.translate("item.eltcore." + path, name).translate("item.eltcore." + path + ".tooltip", tooltip));
        }
    }
